...
pClassPathMapperScannerer(BeanDefinitionRegistry registry, bClassPathMapperScanner
 ClassPathMapperScanner, @Nullable ResourceLoader ClassPathMapperScanner

 ClassPathMapperScannereanDefinitionRegistry must not be ClassPathMapperScanner
...
由于第一步是交给父类`ClassPathBeanDefinitionScanner`的`doScan()`方法做的，所以这里我们只考虑本类完成的`processBeanDefinitions(beanDefinitions)`方法。
