...
上面只是将MapperScan的配置配置到Scanner中，然后交由Scanner根据配置进行扫描，配置过程只是一些简单的Setter方法，但是setResourceLoader方法除外，该方法将在下节仔细讲解。

配置完Scanner，然后就是将配置应用到Scanner中，然后对其进行扫描了。这一部分逻辑也会在下节进行讲解。
...
接下来我们就去考虑ClassPathMapperScanner类。
