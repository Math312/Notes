...
Spring的MapperScan注解用来通过Java配置注册Mybatis，由于这里使用的是tk.mybatis.spring.annotation，但是工作原理与MapperScannerConfigurer相同。考虑其源码如下：
...
根据Spring容器创建规则可以知道，MapperScan的具体逻辑交由`MapperScannerRegistrar`进行处理，下面考察`MapperScannerRegistrar`类。
