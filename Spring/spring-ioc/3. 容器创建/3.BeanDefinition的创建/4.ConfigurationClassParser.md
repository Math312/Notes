...
该方法将各种来源的配置转化成一个`SouceClass`对象。由于在此处正常的被@Configuration直接修饰的配置类或者`Import`引入的配置都会是`StandardAnnotationMetadata`类型，`SourceClass`对象中仅仅保存了`Class对象`或者说是配置类的`source`。
