...
listpack于ziplist的结构几乎一模一样，只是少了一个ztail_offset字段。ziplist通过这个字段来定位最后一个元素的位置，不过listpack可以使用其他方式定位出最后一个元素的位置。listpack的元素结构与ziplist的元素结构很类似，都是包含三个字段，稍有不同的是，前者的长度字段放在了元素的尾部，而存储的不是上一个元素的长，而是当前元素的长度。因为长度放在了尾部，所以可以省去了用于标记最后一个元素位置的zltail_offset字段，最后一个元素可以通过total_bytes字段和最后一个元素的长度字段计算出来。
