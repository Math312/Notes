...
1. bind(final SocketAddress localAddress, final ChannelPromise promise)
    ...
    可以看到，该方法是一个模板方法，该模板方法只是格式化了绑定成功或者失败时的结果返回。在Channel首次激活时异步调用`pipeline.fireChannelActive();`。真正的绑定逻辑在`doBind()`方法中，该方法是一个`abstract方法`。
...
2. register(EventLoop eventLoop, final ChannelPromise promise)
    ...
    同样，该方法是一个模板方法，完成的功能同样还是触发`pipline`对应事件，以及修改异步处理结果。`doRegister()`同样是要求子类具体实现：
...
4. write(Object msg, ChannelPromise promise)
    ...
    同样是模板方法，不过这次没有触发`pipeline`的操作。不过提供了过滤相应数据的方法`filterOutboundMessage(Object msg)`
...
5. flush()
    ...
    这是要介绍的最后一个模板方法，完成的功能同样还是触发`pipline`对应事件，以及修改异步处理结果。`doWrite()`同样是要求子类具体实现。

通过上面的分析，可以发现，`AbstractUnsafe`提供了一系列的模板方法，通过这些模板方法，子类只需要实现网络操作的具体功能，无需关心pipeline事件触发以及异步处理结果响应。
